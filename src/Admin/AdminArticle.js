...
export function ArticlesOverview() {
  const [activeReviewsFilter, setActiveReviewsFilter] = useState('All Articles'); // Renamed for consistency with image
  ...
  const articlesData = [
    ...
    { no: 3, articleId: 'art44', title: 'Watering Schedule Guide', publishDate: '01-01-2025', views: 0, readTime: 0, status: 'Drafted' },
    { no: 4, articleId: 'art1234', title: 'Olive Tree Care', publishDate: '01-01-2025', views: 90, readTime: 0, status: 'Published' },
    { no: 5, articleId: '#ORD0001', title: 'Fixed Amount', publishDate: '01-01-2025', views: 0, readTime: 0, status: 'Drafted' },
    { no: 6, articleId: '#ORD0001', title: 'Percentage', publishDate: '01-01-2025', views: 140, readTime: '3(m)', status: 'Published' },
    { no: 7, articleId: '#ORD0001', title: 'Fixed Amount', publishDate: '01-01-2025', views: 0, readTime: 0, status: 'Drafted' },
  ...
  const filteredArticles = articlesData.filter(article => {
    if (activeReviewsFilter === 'All Articles') return true;
    if (activeReviewsFilter === 'Published' && article.status === 'Published') return true;
    if (activeReviewsFilter === 'Drafted' && article.status === 'Drafted') return true;
    return false;
  });

  // Helper function to return CSS properties for status pills
  const getStatusClasses = (status) => {
    if (status === 'Published') return 'background-color: #D1FAE5; color: #065F46;'; // bg-green-100, text-green-700
    if (status === 'Drafted') return 'background-color: #FEF3C7; color: #92400E;'; // bg-yellow-100, text-yellow-800 - adjusted to fit image
    return '';
  };

  return (
    // العنصر الخارجي: padding متجاوب، وعرض كامل، وإخفاء التجاوز الأفقي للصفحة.
        ...
        <div  className="p-4 sm:p-6 rounded-lg  flex flex-col sm:flex-row justify-between items-start sm:items-center">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            <button
              ...
              onClick={() => setActiveReviewsFilter('All Articles')}
            >
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-medium hover:bg-gray-200"
              onClick={() => setActiveReviewsFilter('Published')}
            >
              Published
            </button>
            <button
              ...
              className="px-4 py-2 text-sm font-medium hover:bg-gray-200"
              onClick={() => setActiveReviewsFilter('Drafted')}
            >
              Drafted
            </button>
          </div>
            ...
            <table className="min-w-full divide-y" style={{ borderColor: '#E5E7EB' }}>
              <thead style={{ backgroundColor: '#F0FDF4' }}>
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#065F46' }}>Article Id</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#065F46' }}>Title</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#065F46' }}>Publish Date</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#065F46' }}>Views</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#065F46' }}>Read Time</th>
